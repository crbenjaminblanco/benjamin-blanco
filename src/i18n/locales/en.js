export default {
  navbar: {
    home: 'Home',
    gallery: 'Gallery',
    calendar: 'Calendar',
    wifi: 'WiFi',
    recommendations: 'Recommendations',
    contact: 'Contact',
    book: 'Book',
    title: 'Benjamin Blanco Calendars'
  },
  hero: {
    title: 'Benjamin Blanco Calendars',
    welcome: 'Welcome to our loft apartment!',
    contact: 'Contact Us',
    scroll: 'Scroll down to explore'
  },
  wifi: {
    title: 'WiFi Connection',
    subtitle: 'High-speed WiFi for remote work and sharing moments',
    networkLabel: 'Network Name',
    passwordLabel: 'Password',
    copyMessage: 'Copied to clipboard!'
  },
  recommendations: {
    title: 'Recommended Places',
    subtitle: 'Discover Zarcero\'s local treasures, from the iconic park to the best restaurants.',
    places: {
      title: 'Points of Interest',
      items: {
        park: {
          name: 'Central Park',
          type: 'Park & Recreation',
          distance: '1 min walking',
          description: 'Beautiful park with trails and playground area.'
        },
        market: {
          name: 'Local Market',
          type: 'Shopping',
          distance: '10 min walking',
          description: 'Traditional market with local products and crafts.'
        },
        plaza: {
          name: 'Main Plaza',
          type: 'Historic Site',
          distance: '6 min walking',
          description: 'Historic plaza with cultural events and architecture.'
        }
      }
    },
    restaurants: {
      title: 'Nearby Restaurants',
      items: {
        terraza: {
          name: 'La Terraza',
          cuisine: 'Mexican • $$',
          distance: '5 min walking',
          description: 'Traditional Mexican cuisine with a modern twist'
        },
        cafe: {
          name: 'Café del Parque',
          cuisine: 'Café • $',
          distance: '2 min walking',
          description: 'Cozy café with fresh pastries and excellent coffee'
        },
        rincon: {
          name: 'El Rincón',
          cuisine: 'Traditional • $$',
          distance: '8 min walking',
          description: 'Local favorite serving authentic dishes'
        }
      }
    }
  },
  social: {
    title: 'Book Your Stay',
    subtitle: 'Choose your preferred platform to secure your place in our loft apartment',
    airbnb: {
      title: 'Book on Airbnb',
      description: 'Check availability and book your stay'
    },
    instagram: {
      title: 'Follow us on Instagram',
      description: 'See more photos and updates',
      embedTitle: 'Benjamin Blanco Calendars on Instagram',
      loadingText: 'Loading Instagram content...'
    },
    whatsapp: {
      title: 'Contact via WhatsApp',
      description: 'Get quick answers to your questions'
    },
    booking: {
      title: 'Book on Booking',
      description: 'Check availability and book your stay'
    }
  },
  gallery: {
    title: 'Gallery',
    subtitle: 'Explore every corner through our gallery',
    items: {
      livingRoom: {
        title: 'Living Room',
        description: 'Bright living room with comfortable seating.'
      },
      bedroom: {
        title: 'Main Bedroom',
        description: 'Cozy bedroom with queen bed and premium bedding.'
      },
      kitchen: {
        title: 'Equipped Kitchen',
        description: 'All necessary appliances and utensils.'
      },
      bathroom: {
        title: 'Full Bathroom',
        description: 'Clean and modern bathroom with shower.'
      },
      parking: {
        title: 'Private Parking',
        description: 'Parking space for small or medium vehicle.'
      },
      exterior: {
        title: 'Exterior View',
        description: 'Beautiful building exterior and surrounding area.'
      }
    }
  },
  thankYou: {
    title: 'Thank You for Your Visit!',
    subtitle: 'We hope you enjoyed your stay at Benjamin Blanco Calendars',
    message: 'We hope to see you again soon at Benjamin Blanco Calendars'
  },
  calendar: {
    title: '🏈 Commanders Game Schedule 2025',
    description: 'Download the calendar with all Commander game nights for 2025. Events will be displayed in your local timezone.',
    chargersTitle: '🏈 Chargers Game Schedule 2025',
    chargersDescription: 'Download the calendar with all Chargers game nights for 2025. Events will be displayed in your local timezone.',
    downloadIOS: 'Download for iOS',
    downloadAndroid: 'Download for Android'
  }
} 